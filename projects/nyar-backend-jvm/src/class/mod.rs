    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, JvmClassError> {
        ...
        for method in &self.methods {
            ...
            let payload = if method.access_flags & (ACC_NATIVE | ACC_ABSTRACT) == 0 {
                ...
                let code = method.code.clone().unwrap_or(build_default_method_code(method, &self.super_name, super_init_methodref)?);
...
fn encode_instruction(
    ...
    match instruction {
        ...
        JvmInstruction::GetStatic(field_ref) => {
            bytes.push(0xB2);
            let descriptor_text = match &field_ref.descriptor {
...
fn encode_lload(index: u16, bytes: &mut Vec<u8>) -> Result<(), JvmClassError> {
    match index {
        ...
        _ => return Err(JvmClassError::InvalidFormat(format!("lload 槽位过大：{index}"))),
    }
    Ok(())
}
...
fn encode_fload(index: u16, bytes: &mut Vec<u8>) -> Result<(), JvmClassError> {
    match index {
        ...
        _ => return Err(JvmClassError::InvalidFormat(format!("fload 槽位过大：{index}"))),
    }
    Ok(())
}
...
fn encode_dload(index: u16, bytes: &mut Vec<u8>) -> Result<(), JvmClassError> {
    match index {
        ...
        _ => return Err(JvmClassError::InvalidFormat(format!("dload 槽位过大：{index}"))),
    }
    Ok(())
}
...
fn encode_lstore(index: u16, bytes: &mut Vec<u8>) -> Result<(), JvmClassError> {
    match index {
        ...
        _ => return Err(JvmClassError::InvalidFormat(format!("lstore 槽位过大：{index}"))),
    }
    Ok(())
}
...
fn encode_fstore(index: u16, bytes: &mut Vec<u8>) -> Result<(), JvmClassError> {
    match index {
        ...
        _ => return Err(JvmClassError::InvalidFormat(format!("fstore 槽位过大：{index}"))),
    }
    Ok(())
}
...
fn encode_dstore(index: u16, bytes: &mut Vec<u8>) -> Result<(), JvmClassError> {
    match index {
        ...
        _ => return Err(JvmClassError::InvalidFormat(format!("dstore 槽位过大：{index}"))),
    }
    Ok(())
}
