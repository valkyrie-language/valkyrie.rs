...
#[test]
fn encodes_double_constants_and_typed_local_ops() {
    let instructions =
        vec![JvmInstruction::LdcDouble(3.5f64.to_bits()), JvmInstruction::DStore(2), JvmInstruction::DLoad(2), JvmInstruction::DReturn];
    let mut pool = ConstantPoolBuilder::new();

    let code = encode_instructions(&instructions, &mut pool).unwrap();

    assert_eq!(code[0], 0x14, "`ldc2_w` 应用于 `double` 常量");
    assert_eq!(code[3], 0x49, "`dstore_2` 应写入双精度局部槽位");
    assert_eq!(code[4], 0x28, "`dload_2` 应从双精度局部槽位读取");
    assert_eq!(code[5], 0xAF, "`dreturn` 应返回双精度结果");
}
