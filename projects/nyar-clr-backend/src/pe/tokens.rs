...
fn resolve_method_ref_token(
    ...
    if is_local {
        ...
        let lookup_key = match method_ref.owner.as_deref() {
            Some(owner) if local_type_names.contains(owner) => format!("{}.{}", owner, method_ref.name),
            _ => method_ref.name.clone(),
        };
        return local_method_tokens
            .get(&lookup_key)
            .or_else(|| local_method_tokens.get(&method_ref.name))
            .copied()
            .ok_or_else(|| PeWriterError::MissingLocalMethodToken(method_ref.name.clone()));
    }
