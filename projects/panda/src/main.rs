//! Panda CLI — clap + nyar-language python + nyar-package-manager.
...
use miette::Report;
use panda::cmds::{
    BuildArgs, ExecArgs, RunArgs, TestArgs,
    ...
    run_build, run_exec, run_run, run_test,
};

#[derive(Debug, Parser)]
#[command(name = "panda", version, about = "Panda — Python 统一工具链（clap + nyar-language python + nyar-package-manager；形态借鉴 Vite+）")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
