...
pub use lowering::{lower_lir_to_jvm_class, JvmLirLoweringLane};
...
/// `JVM` 高层编译请求。
#[derive(Debug)]
pub struct JvmCompileRequest<'a> {
    /// 已选择 lane 的 `LIR`。
...
/// 使用 bundled `JVM` backend 完成完整编译流程。
pub fn compile_jvm_bundle(request: JvmCompileRequest<'_>) -> Result<JvmCompileReport> {
    let lane = JvmLirLoweringLane::new();
    let lowered = lane.lower_partition(request.lir_module)?;
    let entry_class = internal_name_to_binary_name(&lowered.input.internal_name);
    let backend = JvmBinaryBackend::new();
    let input = JvmBinaryBackendInput { class_file: lowered.input, output_dir: request.output_dir, emit_class_file: request.emit_class_file };
    backend.validate(&input)?;
    let artifacts = backend.compile(input, request.options)?;
    Ok(JvmCompileReport { artifacts, entry_class })
}

#[cfg(test)]
mod tests {
    use super::{
        ensure_java_launcher_entry, internal_name_to_binary_name, JvmClassFile, JvmCodeBody, JvmInstruction, JvmMethodDescriptor, JvmMethodRef,
        JvmMethodSignature, JvmTypeDescriptor, JVM_ACC_PUBLIC, JVM_ACC_STATIC,
    };
    ...
    fn creates_java_launcher_for_zero_arg_main() {
        ...
        let launcher = class_file
            ...
            .expect("缺少 Java 启动入口");
        assert_eq!(launcher.access_flags, JVM_ACC_PUBLIC | JVM_ACC_STATIC);
        assert_eq!(
            launcher.code.as_ref().expect("缺少启动器代码").instructions,
            vec![
                JvmInstruction::InvokeStatic(JvmMethodRef {
                    owner: "demo/Main".to_string(),
                    name: "main".to_string(),
                    descriptor: JvmMethodDescriptor::new(Vec::new(), JvmTypeDescriptor::Int),
                }),
                JvmInstruction::Pop,
