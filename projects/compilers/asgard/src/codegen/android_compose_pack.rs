...
pub fn write_vendor_dex_bootstrap(out_dir: &Path) -> Result<()> {
    ...
    let class_defs = vcc_data::binary::dex::dex_class_defs_count(&shell).unwrap_or(0);
    let manifest = format!(
        r#"{{
  "compose": "1.6.0",
  "activity": "1.8.2",
  ...
  "note": "由 write_vendor_dex_bootstrap 生成"
}}
"#
    );
...
mod tests {
    ...
    use vcc_data::binary::dex::dex_contains_strings;
