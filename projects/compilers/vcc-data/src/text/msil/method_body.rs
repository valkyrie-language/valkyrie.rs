...
use super::{MsilInstruction, MsilInstructionOperand, MsilMethodBody, MsilOpcode};
...
impl Diagnostic for MethodBodyError {
    fn code<'a>(&'a self) -> Option<Box<dyn fmt::Display + 'a>> {
        Some(Box::new(match self {
            Self::UnsupportedOperand(_) => "legion::clr::method_body::unsupported_operand",
            Self::OperandMismatch(_) => "legion::clr::method_body::operand_mismatch",
            Self::UndefinedLabel(_) => "legion::clr::method_body::undefined_label",
        }))
    }
...
impl MethodBodyEncoder {
    ...
    fn opcode_size(opcode: MsilOpcode) -> usize {
        if opcode.encoding() > 0xFF { 2 } else { 1 }
    }
        ...
        match &instr.operand {
            ...
            Some(operand) => match operand {
                MsilInstructionOperand::Integer(_) => match instr.opcode {
                    ...
                    MsilOpcode::LdcI4 => 4,
                    MsilOpcode::LdcI8 => 8,
                    MsilOpcode::Ldloc | MsilOpcode::Stloc | MsilOpcode::Ldarg | MsilOpcode::Ldloca => 2,
                    _ => 4,
                },
                ...
                MsilInstructionOperand::BranchTarget(_) => {
                    ...
                    if Self::is_short_branch(instr.opcode) { 1 } else { 4 }
                }
        ...
        match operand {
            MsilInstructionOperand::Integer(value) => {
                ...
                match opcode {
                    ...
                    MsilOpcode::Ldloc | MsilOpcode::Stloc | MsilOpcode::Ldarg | MsilOpcode::Ldloca => {
            ...
            MsilInstructionOperand::Float(_text) => match opcode {
                MsilOpcode::LdcR8 => {
                    let value = _text
                        .parse::<f64>()
                        .map_err(|error| MethodBodyError::OperandMismatch(format!("无效的 `f64` 常量 `{_text}`: {error}")))?;
                    bytes.extend_from_slice(&value.to_le_bytes());
                }
                _ => {
                    return Err(MethodBodyError::UnsupportedOperand(format!("浮点操作数暂不支持指令 `{opcode}`")));
                }
            },
