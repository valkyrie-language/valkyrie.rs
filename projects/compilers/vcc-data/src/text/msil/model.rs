...
impl MsilTypeDef {
    ...
    pub fn qualified_name(&self) -> String {
        if self.namespace.is_empty() { self.full_name.clone() } else { format!("{}.{}", self.namespace, self.full_name) }
    }
}
...
pub struct MsilMethodBody {
    ...
    /// .NET 11+ Runtime Async V2 (`MethodImplAttributes.Async = 0x2000`).
    #[serde(default)]
    pub is_async: bool,
}
...
pub struct MsilMethodSignature {
    ...
    /// 方法是否为实例方法（`HASTHIS` 调用约定，`ECMA-335` II.23.2.1）。
    #[serde(default)]
    pub has_this: bool,
}
...
impl MsilMethodSignature {
    pub fn new(return_type: MsilType, parameter_types: Vec<MsilType>) -> Self {
        Self { return_type, parameter_types, has_this: false }
    }

    /// 创建实例方法签名，`has_this` 为 `true`。
    pub fn new_instance(return_type: MsilType, parameter_types: Vec<MsilType>) -> Self {
        Self { return_type, parameter_types, has_this: true }
    }
...
impl Display for MsilType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Int8 { signed: true } => write!(f, "int8"),
            Self::Int8 { signed: false } => write!(f, "uint8"),
            Self::Int16 { signed: true } => write!(f, "int16"),
            Self::Int16 { signed: false } => write!(f, "uint16"),
            Self::Int32 { signed: true } => write!(f, "int32"),
            Self::Int32 { signed: false } => write!(f, "uint32"),
            Self::Int64 { signed: true } => write!(f, "int64"),
            Self::Int64 { signed: false } => write!(f, "uint64"),
            Self::Float32 => write!(f, "float32"),
            Self::Float64 => write!(f, "float64"),
            Self::String => write!(f, "string"),
            Self::Object => write!(f, "object"),
            Self::IntPtr { signed: true } => write!(f, "IntPtr"),
            Self::IntPtr { signed: false } => write!(f, "UIntPtr"),
...
pub enum MsilOpcode {
    ...
    Tail,
    Ret,
    /// 弹出栈顶值（`pop`，ECMA-335 III.4.32）。
    Pop,
    /// 复制栈顶值（`dup`，ECMA-335 III.4.11）。
    Dup,
    // 控制流：长跳转
    Br,
    Brfalse,
    ...
    LdelemRef,
    LdelemU1,
    /// 按类型从数组加载元素（`ldelem.any`，ECMA-335 III.4.71）——值类型数组必用。
    LdelemAny,
    StelemI4,
    StelemI1,
    StelemRef,
    /// 按类型向数组存储元素（`stelem.any`，ECMA-335 III.4.91）——值类型数组必用。
    StelemAny,
    ...
    /// 存储实例字段值。
    Stfld,
    /// 加载值类型局部变量的地址（`ldloca`，ECMA-335 III.4.77）。
    Ldloca,
    /// 将值类型初始化为全零（`initobj`，ECMA-335 III.4.72）。
    Initobj,
    /// 装箱值类型（`box`，ECMA-335 III.4.1）。
    Box,
    /// 类型转换（`castclass`，ECMA-335 III.4.3）。
    Castclass,
    /// 拆箱为值类型或向下转型引用（`unbox.any`，ECMA-335 III.4.33）。
    UnboxAny,
    /// 从地址加载值类型（`ldobj`，ECMA-335 III.4.74）。
    Ldobj,
    /// 向地址存储值类型（`stobj`，ECMA-335 III.4.50）。
    Stobj,
    /// 内存块复制（`cpblk`，ECMA-335 III.4.9）。
    Cpblk,
}
...
impl FromStr for MsilOpcode {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "ldlen" => Ok(Self::Ldlen),
            "ldelem.i4" => Ok(Self::LdelemI4),
            "ldelem.ref" => Ok(Self::LdelemRef),
            "ldelem.u1" => Ok(Self::LdelemU1),
            "ldelem.any" => Ok(Self::LdelemAny),
            "stelem.i4" => Ok(Self::StelemI4),
            "stelem.i1" => Ok(Self::StelemI1),
            "stelem.ref" => Ok(Self::StelemRef),
            "stelem.any" => Ok(Self::StelemAny),
            ...
            "stfld" => Ok(Self::Stfld),
            "box" => Ok(Self::Box),
            "castclass" => Ok(Self::Castclass),
            "unbox.any" => Ok(Self::UnboxAny),
