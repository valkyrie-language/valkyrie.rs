...
use self::{
    ...
    metadata::{
        FieldRow, ParamRow, TableBuildInfo, UserTypeDefRow, build_local_var_sig, build_method_signature, build_mvid, build_tables_stream,
    },
    relocations::build_relocation_section,
    tokens::resolve_module_tokens,
};
use crate::text::msil::{MethodBodyEncoder, MethodBodyError, MsilModule};

pub use self::{
    meta_builder::{ClrMetadataBuilder, ClrMetadataError, UserStringsHeap},
    method_body::{FatFormat, IlInstruction, LocalVarFlags, LocalVarType, MethodBody, MethodBodyFlags, parse_method_body_il},
    native::{NativeDllImport, NativeImageBuilder, NativePeImage, NativePeWriter},
    read::{
        CliHeader, CoffHeader, DataDirectory, DosHeader, MetadataRoot, OptionalHeader, Pe64Header, Pe64ParseError, PeImage, PeParseError,
        SectionHeader, StreamHeader, TableKind, extract_pe_section, parse_pe, parse_pe64, read_blob, read_compressed_uint, read_strings_string,
        read_user_string, rva_to_offset,
    },
...
impl Diagnostic for PeWriterError {
    fn code<'a>(&'a self) -> Option<Box<dyn fmt::Display + 'a>> {
        Some(Box::new(match self {
            Self::Io(_) => "legion::clr::pe::io",
            Self::MethodBody(_) => "legion::clr::pe::method_body",
            Self::MissingLocalMethodToken(_) => "legion::clr::pe::missing_local_method_token",
            Self::MissingExternalMethodOwner(_) => "legion::clr::pe::missing_external_method_owner",
            Self::ExternalTypeOwnerMissingBracket(_) => "legion::clr::pe::external_type_owner_missing_bracket",
            Self::InvalidExternalTypeOwner(_) => "legion::clr::pe::invalid_external_type_owner",
            Self::UnsupportedExternalTypeOwner(_) => "legion::clr::pe::unsupported_external_type_owner",
            Self::MetadataIndexOverflow { .. } => "legion::clr::pe::metadata_index_overflow",
            Self::NoEntryPoint => "legion::clr::pe::no_entry_point",
            Self::NoMethodBody => "legion::clr::pe::no_method_body",
        }))
    }
...
impl PeWriter {
    ...
    pub fn write_module(&self, module: &MsilModule) -> Result<Vec<u8>, PeWriterError> {
        ...
        let global_methods_type_name_offset = metadata.strings.add("_GlobalMethods");
        ...
        let type_token_map = build_local_type_token_map(&resolved_module);
        let method_flags = build_method_flags(&resolved_module);
        let method_impl_flags = build_method_impl_flags(&resolved_module);
        let user_type_defs = build_user_type_defs(&resolved_module, &mut metadata)?;
        let field_rows = build_field_rows(&resolved_module, &mut metadata, &type_token_map)?;

        // Param 表：为带形参的方法写入 ECMA 合规的 Param 行。
        let (param_rows, method_param_lists) = build_param_tables(&resolved_module);
        ...
        for type_def in &resolved_module.types {
            for method in &type_def.methods {
                ...
                method_bodies.push(MethodBodyEncoder::encode(method, local_var_sig_token).map_err(PeWriterError::MethodBody)?);
                method_name_offsets.push(metadata.strings.add(&method.method.name));
                // Instance `.ctor` must always carry HASTHIS — even if owner was dropped —
                // otherwise MethodAttributes (0x1886, no Static) disagree with the blob and
                // CoreCLR raises TypeLoadException: "Incorrect signature".
                let has_this = method.method.name == ".ctor" || (method.method.owner.is_some() && !is_static_type_method(method));
                method_sig_offsets.push(metadata.blob.add(&build_method_signature(&method.method.signature, has_this, &type_token_map)));
            }
        }
        ...
        let table_info = TableBuildInfo {
            ...
            global_method_count: resolved_module.global_methods.len() as u32,
            global_methods_type_name_offset,
            ...
            type_spec_rows: &resolved.type_spec_rows,
            ...
            method_param_lists: &method_param_lists,
        };
...
fn find_entry_method_lookup_key(module: &MsilModule, _module_name: &str) -> Option<String> {
    if let Some(method) = module.global_methods.iter().find(|m| m.is_entry_point) {
        return Some(
            method.method.owner.as_ref().map(|owner| format!("{}.{}", owner, method.method.name)).unwrap_or_else(|| method.method.name.clone()),
        );
    }
...
fn build_local_type_token_map(module: &MsilModule) -> metadata::TypeTokenMap {
    let mut tokens = BTreeMap::new();
    ...
    for (index, type_def) in module.types.iter().enumerate() {
        ...
        let qualified_name = type_def.qualified_name();
        tokens.insert(qualified_name.clone(), token);
        if type_def.is_value_type {
            value_type_names.insert(type_def.full_name.clone());
            value_type_names.insert(qualified_name);
            value_type_tokens.insert(token);
        }
    }
    metadata::TypeTokenMap { tokens, value_type_names, value_type_tokens }
}

fn build_method_flags(module: &MsilModule) -> Vec<u16> {
    let mut flags = Vec::new();
    for method in &module.global_methods {
        flags.push(global_method_flags_for(method));
    }
    for type_def in &module.types {
        for method in &type_def.methods {
            flags.push(type_method_flags_for(method));
        }
    }
    flags
}

fn build_method_impl_flags(module: &MsilModule) -> Vec<u16> {
    let mut flags = Vec::new();
    for method in &module.global_methods {
        flags.push(method_impl_flags_for(method));
    }
    for type_def in &module.types {
        for method in &type_def.methods {
            flags.push(method_impl_flags_for(method));
...
fn global_method_flags_for(method: &crate::text::msil::MsilMethodBody) -> u16 {
    // Module-level methods are always static. Never apply instance `.ctor` flags (0x1886)
    // here — that pairs with has_this=false blobs and triggers TypeLoadException on
    // the synthetic `_GlobalMethods` type ("Incorrect signature").
    match method.method.name.as_str() {
        ".cctor" => 0x1891,
        _ => 0x0096,
    }
}

fn type_method_flags_for(method: &crate::text::msil::MsilMethodBody) -> u16 {
    // MethodAttributes: Public | HideBySig [| Static] [| SpecialName | RTSpecialName]
    if method.method.name == ".cctor" {
        0x1891 // Public | Static | HideBySig | SpecialName | RTSpecialName
    }
    else if method.method.name == ".ctor" {
        0x1886 // Public | HideBySig | SpecialName | RTSpecialName (instance — never Static)
    }
    else if method.method.owner.is_some() && !is_static_type_method(method) {
        0x0086 // Public | HideBySig (instance)
    }
    else {
        0x0096 // Public | Static | HideBySig
    }
}

/// 判断类型方法是否为静态方法。
///
/// CLR 类型方法中，`.cctor`（静态构造函数）和 singleton accessor
/// （`instance` / `get_instance`）是静态方法，其余为实例方法。
/// 静态方法的 `MethodAttributes` 必须包含 `Static` (0x0010)，
/// 且方法签名不得包含 `HASTHIS` (0x20) 标志，否则运行时会抛出
/// `TypeLoadException: The signature is incorrect.`
fn is_static_type_method(method: &crate::text::msil::MsilMethodBody) -> bool {
    matches!(method.method.name.as_str(), ".cctor" | "instance" | "get_instance")
}

fn method_impl_flags_for(method: &crate::text::msil::MsilMethodBody) -> u16 {
    // MethodImplAttributes.Async = 0x2000 (.NET 11 Runtime Async V2)
    if method.is_async { 0x2000 } else { 0 }
}

fn build_user_type_defs(module: &MsilModule, metadata: &mut ClrMetadataBuilder) -> Result<Vec<UserTypeDefRow>, PeWriterError> {
    let mut rows = Vec::with_capacity(module.types.len());
    let mut next_field_row = 1u16;
    let mut next_method_row = (module.global_methods.len() as u16).saturating_add(1);
    for type_def in &module.types {
        let type_name_offset = metadata.strings.add(&type_def.full_name);
        let type_namespace_offset = metadata.strings.add(&type_def.namespace);
        rows.push(UserTypeDefRow {
            type_name_offset,
            type_namespace_offset,
            field_list: next_field_row,
            method_list: next_method_row,
            is_value_type: type_def.is_value_type,
        });
        next_field_row = next_field_row.saturating_add(type_def.fields.len() as u16);
        next_method_row = next_method_row.saturating_add(type_def.methods.len() as u16);
    }
    Ok(rows)
}

fn build_param_tables(module: &MsilModule) -> (Vec<ParamRow>, Vec<u16>) {
    let mut param_rows = Vec::new();
    let mut method_param_lists = Vec::new();
    let mut append_method = |method: &crate::text::msil::MsilMethodBody| {
        let param_count = method.method.signature.parameter_types.len();
        if param_count == 0 {
            method_param_lists.push((param_rows.len() as u16).saturating_add(1));
            return;
        }
        method_param_lists.push((param_rows.len() as u16).saturating_add(1));
        for sequence in 1..=param_count as u16 {
            param_rows.push(ParamRow { flags: 0, sequence, name_offset: 0 });
        }
    };
    for method in &module.global_methods {
    ...
    (param_rows, method_param_lists)
}

fn build_field_rows(
    module: &MsilModule,
    metadata: &mut ClrMetadataBuilder,
    type_token_map: &metadata::TypeTokenMap,
) -> Result<Vec<FieldRow>, PeWriterError> {
    let mut rows = Vec::new();
    for type_def in &module.types {
        for field in &type_def.fields {
            let name_offset = metadata.strings.add(&field.name);
            let signature_offset = metadata.blob.add(&metadata::build_field_signature(&field.ty, type_token_map));
            let flags = if field.is_static { 0x0016u16 } else { 0x0006u16 };
            rows.push(FieldRow { flags, name_offset, signature_offset });
        }
    }
    Ok(rows)
}
