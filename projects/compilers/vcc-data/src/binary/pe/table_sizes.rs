//! 元数据表行大小计算器。
//!
//! 根据 `ECMA-335` II.22 计算各元数据表的单行字节数与表数据偏移，
//! 用于在 `#~` 流中精确定位目标表，避免依赖脆弱的顺序 `cursor` 推进
//! （中间穿插未知表时会错位）。
//!
//! 行大小取决于：
...
use super::read::MetadataRoot;

/// 编码索引类型。
///
/// `ECMA-335` II.24.2.6 定义的 13 种编码索引，每种由若干表共享并通过 `tag` 位区分。
...
impl CodedIndex {
    ...
    /// 返回该编码索引引用的所有表种类（按 `tag` 顺序）。
    pub fn referenced_tables(self) -> &'static [u8] {
        match self {
...
/// 计算编码索引的字节大小（2 或 4）。
///
/// 阈值 = `1 << (16 - tag_bits)`，当引用表的最大行数达到阈值时升级为 4 字节。
pub fn coded_index_size(md: &MetadataRoot, coded: CodedIndex) -> usize {
...
/// 计算简单表索引的字节大小（2 或 4）。
pub fn table_index_size(md: &MetadataRoot, table_id: u8) -> usize {
    if md.row_counts[table_id as usize] > 0xFFFF { 4 } else { 2 }
}

/// 堆索引大小。
///
/// 当堆数据超过 `0xFFFF` 字节时使用 4 字节索引，否则 2 字节。
pub fn heap_index_size(heap_size: usize) -> usize {
    if heap_size > 0xFFFF { 4 } else { 2 }
}

/// 计算指定表种类的单行字节数。
///
/// 返回 `Some(size)` 表示已知结构，`None` 表示未实现的表种类。
...
/// 计算指定表在 `#~` 流表数据区中的字节偏移。
///
/// 从表种类 `0` 开始顺序累加各已存在表的总字节数，直到遇到目标表。
