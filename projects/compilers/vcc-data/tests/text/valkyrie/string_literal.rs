...
fn parses_triple_quoted_interpolation() {
    ...
    let function = match &root.statements[0] {
        vcc_data::text::valkyrie::RootStatement::Function(function) => function,
        _ => panic!("expected function"),
    };
    let body = function.body.as_ref().expect("body");
    let expr = match &body.statements[0] {
        vcc_data::text::valkyrie::FunctionStatement::Let(let_stmt) => let_stmt.initializer.as_ref().expect("initializer"),
