use vcc_data::text::valkyrie::{AstParser, FunctionStatement, ParameterBindingKind, RootStatement, TermExpression};

fn first_micro_params(source: &str) -> Vec<vcc_data::text::valkyrie::FunctionParameter> {
...
fn first_call_arguments(source: &str) -> Vec<vcc_data::text::valkyrie::TermCallArgument> {
    let root = AstParser::parse_root(source).expect("parse");
