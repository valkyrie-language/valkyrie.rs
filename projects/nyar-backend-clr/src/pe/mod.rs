...
use self::{
    ...
    metadata::{build_local_var_sig, build_method_signature, build_mvid, build_tables_stream, FieldRow, TableBuildInfo, UserTypeDefRow},
    ...
    pub fn write_module(&self, module: &MsilModule) -> Result<Vec<u8>, PeWriterError> {
        ...
        for type_def in &resolved_module.types {
            for method in &type_def.methods {
                ...
                method_bodies.push(MethodBodyEncoder::encode(method, local_var_sig_token).map_err(PeWriterError::MethodBody)?);
                method_name_offsets.push(metadata.strings.add(&method.method.name));
                let has_this = method.method.name == ".ctor";
...
fn build_local_type_token_map(module: &MsilModule) -> BTreeMap<String, u32> {
    ...
    for (index, type_def) in module.types.iter().enumerate() {
        let token = 0x0200_0000 | ((index as u32) + 2);
        tokens.insert(type_def.full_name.clone(), token);
        let qualified_name = type_def.qualified_name();
        tokens.insert(qualified_name, token);
    }
    tokens
}
...
fn build_method_flags(module: &MsilModule) -> Vec<u16> {
    ...
    for method in &module.global_methods {
        flags.push(method_flags_for(method));
    }
    for type_def in &module.types {
        for method in &type_def.methods {
            flags.push(method_flags_for(method));
...
fn method_flags_for(method: &crate::msil::MsilMethodBody) -> u16 {
    if method.method.name == ".ctor" {
        0x1886
    }
    else {
        0x0096
...
fn build_user_type_defs(module: &MsilModule, metadata: &mut ClrMetadataBuilder) -> Result<Vec<UserTypeDefRow>, PeWriterError> {
    ...
    for type_def in &module.types {
        let type_name_offset = to_u16(metadata.strings.add(&type_def.full_name), "TypeDef 类型名")?;
        let type_namespace_offset = to_u16(metadata.strings.add(&type_def.namespace), "TypeDef 命名空间")?;
...
fn build_field_rows(
    ...
    type_token_map: &BTreeMap<String, u32>,
) -> Result<Vec<FieldRow>, PeWriterError> {
    let mut rows = Vec::new();
    for type_def in &module.types {
        for field in &type_def.fields {
            let name_offset = to_u16(metadata.strings.add(&field.name), "Field 名称")?;
            let signature_offset = to_u16(metadata.blob.add(&metadata::build_field_signature(&field.ty, type_token_map)), "Field 签名")?;
            rows.push(FieldRow { flags: 0x0006, name_offset, signature_offset });
        }
    }
    Ok(rows)
}
