...
use crate::{
    lir::{LirBlock, LirEffectKind, LirFunction, LirModule, LirOperand, LirTerminator},
    mir::{MirConstant, MirEffectKind, MirValueRef},
};
...
fn validate_function(function: &LirFunction) -> Result<(), ParseError> {
    ...
    if !blocks.contains_key(&function.entry) {
        return Err(ParseError::invalid(format!("控制流调度校验失败：`LIR` 函数 `{}` 的 entry block 不存在", function.symbol)));
    }
    ...
    for block in &function.blocks {
        ...
        match &block.terminator {
            ...
            LirTerminator::PerformEffect { effect, payload, resume_target, .. } => {
                ...
                let mir_effect = lower_effect_kind(*effect);
                if let Some(payload_type) = payload.as_ref().and_then(|payload| infer_operand_static_type(function, payload)) {
                    validate_effect_payload_static_type(&function.symbol, block.label.as_str(), mir_effect, &payload_type)?;
                }
    ...
    validate_runtime_frames(function)?;
    validate_runtime_continuations(function)?;
    Ok(())
}

fn validate_suspend_points(function: &LirFunction, block_map: &BTreeMap<crate::mir::MirBlockRef, &LirBlock>) -> Result<(), ParseError> {
...
fn validate_frame_layout(
    ...
    index: usize,
    layout: &crate::lir::LirFrameLayout,
) -> Result<(), ParseError> {
    let Some(suspend_point) = function.suspend_points.iter().find(|suspend_point| suspend_point.state_id == layout.state_id)
    else {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 frame layout 找不到对应的 suspend 点",
            index + 1
        )));
    };
    if suspend_point.resume_target != layout.resume_target {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 frame layout 恢复目标与 suspend 点不一致",
            index + 1
        )));
    }
    if suspend_point.spill_candidates.len() != layout.slots.len() {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 frame layout 槽位数量与 suspend spill 集不一致",
            index + 1
        )));
    }
    for (slot_index, (expected_value, slot)) in suspend_point.spill_candidates.iter().zip(layout.slots.iter()).enumerate() {
        if slot.slot_index != slot_index || slot.value != *expected_value {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 frame layout 第 {} 个槽位未对齐 suspend spill 顺序",
                index + 1,
                slot_index + 1
            )));
        }
        if let (Some(expected_type), Some(actual_type)) = (function.value_types.get(&slot.value), slot.value_type.as_ref()) {
            if expected_type != actual_type {
                return Err(ParseError::invalid(format!(
                    "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 frame layout 第 {} 个槽位类型为 `{}`，但 SSA 值类型为 `{}`",
                    index + 1,
                    slot_index + 1,
...
fn validate_continuations(function: &LirFunction, block_map: &BTreeMap<crate::mir::MirBlockRef, &LirBlock>) -> Result<(), ParseError> {
...
fn validate_case_chains(function: &LirFunction, block_map: &BTreeMap<crate::mir::MirBlockRef, &LirBlock>) -> Result<(), ParseError> {
    for (index, case_chain) in function.case_chains.iter().enumerate() {
        ...
        {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 指向了不存在的 block",
                function.symbol,
                index + 1
            )));
        }
        if case_chain.arms.is_empty() {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 不允许缺少 arm",
                function.symbol,
                index + 1
            )));
        }
        if case_chain.arms.first().map(|arm| arm.entry_block) != Some(case_chain.first_arm) {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 首 arm 入口与记录不一致",
                function.symbol,
                index + 1
            )));
        }
        for (arm_index, arm) in case_chain.arms.iter().enumerate() {
            if !block_map.contains_key(&arm.entry_block) || !block_map.contains_key(&arm.body_block) {
                return Err(ParseError::invalid(format!(
                    "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 第 {} 个 arm 指向了不存在的入口或 body block",
                    function.symbol,
                    index + 1,
                    arm_index + 1
                )));
            ...
            if let Some(check_block) = arm.check_block {
                if !block_map.contains_key(&check_block) {
                    return Err(ParseError::invalid(format!(
                        "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 第 {} 个 arm 指向了不存在的 check block",
                        function.symbol,
                        index + 1,
                        arm_index + 1
                    )));
            ...
            if let Some(guard_block) = arm.guard_block {
                if !block_map.contains_key(&guard_block) {
                    return Err(ParseError::invalid(format!(
                        "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 第 {} 个 arm 指向了不存在的 guard block",
                        function.symbol,
                        index + 1,
                        arm_index + 1
                    )));
            ...
            if !block_map.contains_key(&arm.next_arm_target) || !block_map.contains_key(&arm.exit_target) {
                return Err(ParseError::invalid(format!(
                    "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 第 {} 个 arm 指向了不存在的后继或 exit block",
                    function.symbol,
                    index + 1,
                    arm_index + 1
                )));
            ...
            if let Some(fallthrough_target) = arm.fallthrough_target {
                if !block_map.contains_key(&fallthrough_target) {
                    return Err(ParseError::invalid(format!(
                        "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 case chain 第 {} 个 arm 指向了不存在的 fallthrough 目标",
                        function.symbol,
                        index + 1,
                        arm_index + 1
                    )));
...
fn validate_runtime_frames(function: &LirFunction) -> Result<(), ParseError> {
    if function.runtime_frames.len() != function.frame_layouts.len() {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：`LIR` 函数 `{}` 的 runtime frame 数量与 frame layout 数量不一致",
            function.symbol
        )));
    }
    let mut carriers = BTreeSet::new();
    for (index, (runtime_frame, layout)) in function.runtime_frames.iter().zip(function.frame_layouts.iter()).enumerate() {
        if runtime_frame.carrier.is_empty() {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 runtime frame 缺少 carrier 名称",
                function.symbol,
                index + 1
            )));
        }
        if !carriers.insert(runtime_frame.carrier.as_str()) {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的 runtime frame carrier `{}` 重复",
                function.symbol, runtime_frame.carrier
            )));
        }
        if runtime_frame.state_id != layout.state_id || runtime_frame.resume_target != layout.resume_target {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 runtime frame 与 frame layout 不一致",
                function.symbol,
                index + 1
            )));
        }
        if runtime_frame.slots.len() != layout.slots.len() {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 runtime frame 槽位数量与 frame layout 不一致",
                function.symbol,
                index + 1
            )));
        }
        for (slot_index, (runtime_slot, layout_slot)) in runtime_frame.slots.iter().zip(layout.slots.iter()).enumerate() {
            let expected_field = format!("slot_{}", layout_slot.slot_index);
            if runtime_slot.field_name != expected_field
                || runtime_slot.slot_index != layout_slot.slot_index
                || runtime_slot.value != layout_slot.value
                || runtime_slot.value_type != layout_slot.value_type
            {
                return Err(ParseError::invalid(format!(
                    "控制流调度校验失败：`LIR` 函数 `{}` 的第 {} 个 runtime frame 第 {} 个槽位与 frame layout 不一致",
                    function.symbol,
                    index + 1,
                    slot_index + 1
                )));
...
fn validate_runtime_continuations(function: &LirFunction) -> Result<(), ParseError> {
    if function.runtime_continuations.len() != function.continuations.len() {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：`LIR` 函数 `{}` 的 runtime continuation 数量与 continuation 数量不一致",
            function.symbol
        )));
    }
    let mut carriers = BTreeSet::new();
    for (index, (runtime_continuation, continuation)) in function.runtime_continuations.iter().zip(function.continuations.iter()).enumerate() {
        ...
        if !carriers.insert(runtime_continuation.carrier.as_str()) {
            return Err(ParseError::invalid(format!(
                "控制流调度校验失败：`LIR` 函数 `{}` 的 runtime continuation carrier `{}` 重复",
                function.symbol, runtime_continuation.carrier
            )));
        }
        if runtime_continuation.resume_parameter_field.is_empty()
            || runtime_continuation.resume_parameter_field != "resume_value"
            || runtime_continuation.dispatch_block != continuation.dispatch_block
...
fn validate_continuation_target(
    ...
    blocks: &BTreeMap<crate::mir::MirBlockRef, &LirBlock>,
) -> Result<(), ParseError> {
    let Some(resume_block) = blocks.get(&resume_target)
    else {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 continuation 指向了不存在的 resume block",
            index + 1
        )));
    };
    if !blocks.contains_key(&dispatch_block) {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 continuation 指向了不存在的 dispatch block",
            index + 1
        )));
    }
    if !blocks.contains_key(&handler_exit) {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 continuation 指向了不存在的 handler exit block",
            index + 1
        )));
    }
    if !resume_block.parameters.contains(&resume_parameter) {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 continuation 恢复参数不属于目标 resume block",
            index + 1
        )));
    }
    Ok(())
...
fn validate_suspend_target(
    function_name: &str,
    index: usize,
    suspend_point: &crate::lir::LirSuspendPoint,
    blocks: &BTreeMap<crate::mir::MirBlockRef, &LirBlock>,
) -> Result<(), ParseError> {
    if !blocks.contains_key(&suspend_point.suspend_block) {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 suspend 点指向了不存在的挂起 block",
            index + 1
        )));
    }
    let Some(resume_block) = blocks.get(&suspend_point.resume_target)
        ...
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 suspend 点指向了不存在的恢复 block",
            index + 1
        )));
    };
    if resume_block.parameters.len() != suspend_point.resume_parameter_count {
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：函数 `{function_name}` 的 `LIR` 第 {} 个 suspend 点恢复参数个数为 {}，但目标恢复 block 实际有 {} 个参数",
            index + 1,
...
fn collect_reachable_blocks(function: &LirFunction) -> BTreeSet<crate::mir::MirBlockRef> {
...
fn validate_jump_target(
    ...
    blocks: &BTreeMap<crate::mir::MirBlockRef, usize>,
    block_map: &BTreeMap<crate::mir::MirBlockRef, &LirBlock>,
) -> Result<(), ParseError> {
...
fn validate_effect_payload_static_type(
        ...
        && future_resume_type(payload_type).is_none()
    {
        ...
        return Err(ParseError::invalid(format!(
            "控制流调度校验失败：`LIR` 函数 `{function_name}` 的 block `{block_label}` 中 {effect_name} 的 effect payload 类型 `{}` 不满足 `Future<T>` / `Promise<T>` 形状",
...
fn lower_effect_kind(effect: LirEffectKind) -> MirEffectKind {
    match effect {
