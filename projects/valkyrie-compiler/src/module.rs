...
mod tests {
    ...
    fn test_add_module() {
        ...
        graph.add_module(QualifiedName::from("a"));
        assert!(!graph.is_empty());
        assert_eq!(graph.len(), 1);
        assert!(graph.contains(&QualifiedName::from("a")));
    }

    #[test]
    fn test_add_dependency_auto_adds_modules() {
        let mut graph = DependencyGraph::new();
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("b"));
        assert_eq!(graph.len(), 2);
        assert!(graph.contains(&QualifiedName::from("a")));
        assert!(graph.contains(&QualifiedName::from("b")));
    }
    ...
    fn test_dependencies() {
        let mut graph = DependencyGraph::new();
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("b"));
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("c"));

        let deps = graph.dependencies(&QualifiedName::from("a")).unwrap();
        assert_eq!(deps.len(), 2);
        assert!(deps.contains(&QualifiedName::from("b")));
        assert!(deps.contains(&QualifiedName::from("c")));
    }
    ...
    fn test_dependents() {
        let mut graph = DependencyGraph::new();
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("c"));
        graph.add_dependency(QualifiedName::from("b"), QualifiedName::from("c"));

        let dependents = graph.dependents(&QualifiedName::from("c"));
        assert_eq!(dependents.len(), 2);
        assert!(dependents.contains(&QualifiedName::from("a")));
        assert!(dependents.contains(&QualifiedName::from("b")));
    }
    ...
    fn test_simple_cycle() {
        let mut graph = DependencyGraph::new();
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("b"));
        graph.add_dependency(QualifiedName::from("b"), QualifiedName::from("a"));
    ...
    fn test_self_cycle() {
        let mut graph = DependencyGraph::new();
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("a"));
        assert!(graph.detect_cycle().is_some());
    }
    ...
    fn test_topological_sort_order() {
        ...
        let c_pos = sorted.iter().position(|n| *n == QualifiedName::from("c")).unwrap();
        let b_pos = sorted.iter().position(|n| *n == QualifiedName::from("b")).unwrap();
        let a_pos = sorted.iter().position(|n| *n == QualifiedName::from("a")).unwrap();
        assert!(c_pos < b_pos);
        assert!(b_pos < a_pos);
    }
    ...
    fn test_diamond_dependency() {
        ...
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("b"));
        graph.add_dependency(QualifiedName::from("a"), QualifiedName::from("c"));
        graph.add_dependency(QualifiedName::from("b"), QualifiedName::from("d"));
        graph.add_dependency(QualifiedName::from("c"), QualifiedName::from("d"));

        let sorted = graph.topological_sort().unwrap();
        let d_pos = sorted.iter().position(|n| *n == QualifiedName::from("d")).unwrap();
        let b_pos = sorted.iter().position(|n| *n == QualifiedName::from("b")).unwrap();
        let c_pos = sorted.iter().position(|n| *n == QualifiedName::from("c")).unwrap();
        let a_pos = sorted.iter().position(|n| *n == QualifiedName::from("a")).unwrap();
        assert!(d_pos < b_pos);
    ...
    fn test_remove_module() {
        ...
        assert!(graph.remove_module(&QualifiedName::from("b")));
        assert!(!graph.contains(&QualifiedName::from("b")));
        assert!(graph.dependencies(&QualifiedName::from("a")).unwrap().is_empty());
    }
    ...
    fn test_qualified_names_with_namespace() {
        let mut graph = DependencyGraph::new();
        graph.add_dependency(QualifiedName::from("std::collections::HashMap"), QualifiedName::from("std::hash::Hash"));
        graph.add_dependency(QualifiedName::from("std::collections::HashSet"), QualifiedName::from("std::hash::Hash"));
