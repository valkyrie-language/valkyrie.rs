...
function gateOrSkip(t: test.TestContext): boolean {
    if (integrationRunnerReady(REPO_ROOT, WASM_COLLECT, WASM_ENTRY)) {
        return true;
    }
    if (integrationRequired()) {
        assert.fail("LEGION_INTEGRATION=1 requires wasm collect (pnpm assemble) or target/release/legion.exe");
    }
    t.skip("no wasm collect and no native legion.exe");
    return false;
}
...
test("integration gate documents wasm or native runner", () => {
    const ready = integrationRunnerReady(REPO_ROOT, WASM_COLLECT, WASM_ENTRY);
    if (ready) {
        assert.ok(true);
        return;
    }
...
test("legion build minimal-node → canonical legion.mjs/wasm → runtime exit 0", { timeout: BUILD_TIMEOUT_MS }, (t) => {
    ...
    try {
        const build = spawnLegionForIntegration(REPO_ROOT, host, [
            "build",
        ...
        assert.equal(build.route, "native", "integration build must use native seed legion");
