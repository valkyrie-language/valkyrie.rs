...
theorem model_checking_decidable<M: KripkeStructure, φ: CTL.Formula>() -> 
    ...
    proof {
        ...
        # 证明算法的正确性和终止性
        have correctness: AlgorithmCorrect(algorithm) = {
            structural_induction_on_formula(φ)
        }
        
        have termination: AlgorithmTerminates(algorithm) = {
...
theorem quicksort_correctness<A: TotalOrder>(
    ...
    HoareTriple {
        precondition: λarr. True,  # 无前置条件
        program: quicksort,
        postcondition: λresult. 
            Sorted(result) ∧ 
        ...
        validity: proof {
            # 结构归纳证明
            induction on arr.length {
                case 0 => trivial,
                case 1 => trivial,
                case succ(succ(n)) => {
                    # 分治递归情况
                    let pivot = arr[0]
                    let (left, right) = partition(arr[1..], pivot)
                    
                    # 归纳假设
                    have ih_left: Sorted(quicksort(left)) ∧ 
                                 Permutation(left, quicksort(left))
                    have ih_right: Sorted(quicksort(right)) ∧ 
                                  Permutation(right, quicksort(right))
...
theorem mutex_correctness(
    ...
    proof {
        # 安全性：互斥访问
        have mutual_exclusion: 
            ∀(t: Time). AtMostOneProcess(InCriticalSection(t))
        
        # 活性：无死锁
        have deadlock_freedom: 
            ∀(trace: ExecutionTrace). 
            ProcessWaiting(trace) → EventuallyEnters(trace)
        
        # 公平性：无饥饿
        have starvation_freedom: 
            ∀(p: Process). 
