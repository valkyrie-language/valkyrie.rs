...
class Dog(Animal) {
    ...
    initiate(mut self, name: utf8, age: i32, breed: utf8) {
        self.name = name
        self.age = age
        self.breed = breed
    }
...
class Circle {
    ...
    initiate(mut self, radius: f64) {
        self.radius = radius
    }
}

imply Circle: Drawable {
    draw(self) {
...
imply Circle: Comparable⟨Circle⟩ {
...
class Rectangle {
    ...
    initiate(mut self, width: f64, height: f64) {
        self.width = width
        self.height = height
    }
...
loop (index, value) in collection.enumerate() {
...
# 引用计数
let shared_data = Rc { }
shared_data.initiate(expensive_data())
