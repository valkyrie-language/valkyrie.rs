...
- **CLR / JVM / WASM** 的正式生成由各自手写二进制路径完成（见 [clr.md](clr.md)、[jvm.md](jvm.md)、[wasi.md](wasi.md)），
  **不**经 Gaia。

## 约束对齐

即便扩展 native，仍须遵守：无语言 `string`（用 utf8/utf16）；`void`≠`unit`；若产出与托管目标重叠，不得改走 ilasm/dnlib/javac
等外部生成器。
