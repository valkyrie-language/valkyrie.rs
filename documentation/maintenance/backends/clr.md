...
CLR (Common Language Runtime) 使用基于栈的 CIL。本仓库的正式生成路径是： **MIR → 类型化 MSIL 模型 → 手写 PE/CLI 二进制**，与
`valkyrie.v` 侧 `std.data.binary.pe` 同构。
...
- **二进制**：`../../../projects/vcc-data`（`PeWriter`；模块注释明确「不依赖 ilasm」）。
- **MSIL 文本模型**：`../../../projects/vcc-data`（编码方法体与元数据行，再由 PE writer 落盘）。
...
- `void`（ADT 代数 0，平台层常为 `NyarType::Bottom`）与 `unit`（ADT 代数 1，`NyarType::Unit`） **禁止混淆**。返回签名在 CIL
  上可能都编码为 `void`，但语言/IR 语义仍须区分；`LocalVarSig` 不得使用 `ELEMENT_TYPE_VOID`（ECMA-335）。
