...
> **注意**: 诸如 `vendor` (供应商)、`registry` (注册表) 等术语是 Legion 包管理器的特定概念，而非 Valkyrie
> 语言本身定义的语义。Valkyrie 语言只关注符号的逻辑组织，而 Legion 负责将这些逻辑符号映射到物理存储（如 `vendor` 目录）或远程服务（如
> `jsr`, `npm` 等 registry）。
...
由项目根目录下的 **`legion.von`** 驱动（VON 清单；源码 `legion` planner 查找此文件名）。

- **适用场景**: 独立的库或应用程序。
- **核心逻辑**: Legion 以 `legion.von` 所在目录为根，解析其 `dependencies` 字段，并管理该项目私有的 `vendor` 缓存（如果存在）。
- **标识**: 配置文件中定义 `name` 和 `version`，作为该包在符号系统中的顶级命名空间。

#### 工作区模式 (Workspace Mode)

由根目录下的 **`legions.von`**（注意是复数）驱动。

- **适用场景**: 包含多个相互关联包的大型项目（Monorepo）。
- **核心逻辑**:
    - `legions.von` 定义了工作区的成员路径（`members`）。
    - 成员包之间可以直接通过包名互相引用，无需在各自的 `legion.von` 中声明相对路径依赖。
    - 工作区共享统一的 `vendor` 目录，避免重复下载相同的依赖。
- **属性继承**: 成员包可以自动从 `legions.von` 中继承通用属性（如 `version`, `description`, `authors`, `license`
  等），无需在子项目中重复声明。
- **依赖提升**: 工作区级的依赖定义会作为所有成员包的后备（Fallback），即解析顺序中的“工作区依赖”。成员包通过将依赖项的值设为
  `true` 来明确声明使用工作区统一定义的版本。

- **职责**: 发现工作区项目、解析依赖版本、管理 `vendor` 目录、提供包名到物理路径的映射。
- **依赖解析顺序**:
    1. **自身项目**: 首先检查当前项目内的文件。
    2. **项目依赖**: 检查 `legion.von` 中定义的直接依赖。
    3. **工作区依赖**: 检查 `legions.von` 中定义的共享依赖。
    4. **供应商缓存**: 在 `%LEGION_ROOT%/vendor/` 目录下递归搜索匹配的版本。

> **更正**: 旧文档曾写 `legion.json` / `legions.json` / `legion.toml`。现行清单扩展名为 **`.von`**。
...
4. **按需索引**: LSP 获取该目录下的所有 `.v` 源文件，并将其编译进符号表中。
...
```text
my_project/
├── legion.von           # 必须：项目元数据与依赖定义
├── library/             # 核心：库源代码目录
│   ├── _.v             # 建议：命名空间根目录定义
│   └── utils.v         # 子模块 (对应命名空间 my_project::utils)
├── binary/              # 运行：可执行指令目录
│   ├── simple.v        # 注册为指令 `simple`
│   └── complex/
│       └── main.v      # 注册为指令 `complex`
...
- **`library/`**: 存放核心库代码。如果存在，Legion 将仅从此目录加载作为库成员的 **`.v`** 文件（语言名 Valkyrie，后缀 `.v`
  ——同一事物；勿再使用历史后缀 `.vk` / `.valkyrie` 作为平行后缀）。
- **`binary/`**: 存放可执行程序。
    - 文件如 `binary/simple.v` 会被自动注册为与文件名同名的指令（`simple`）。
    - 目录如 `binary/complex/main.v` 会被注册为目录名同名的指令（`complex`）。
...
- **行为**: 指令会解析包的版本信息，将其写入 **`legion.von`** 的依赖区域。
...
在项目根目录下运行 `legion install`，它会根据 `legion.von` 或 `legions.von` 的定义，将所有声明的依赖下载并同步到项目的
`vendor/` 目录下。
...
- **文件监听**: 监听你包管理器的特定配置文件（现行为 `legion.von` / `legions.von`），并在更改时通知 LSP 刷新。
