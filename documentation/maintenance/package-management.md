...
- **属性继承**: 成员包可以自动从 `legions.von` 中继承通用属性（如 `version`, `description`, `authors`, `license` 等），无需在子项目中重复声明。
...
### 4.1 核心目录行为
- **`library/`**: 存放核心库代码。如果存在，Legion 将仅从此目录加载作为库成员的 **`.v`** 文件（语言名 Valkyrie，后缀 `.v`——同一事物；勿再使用历史后缀 `.vk` / `.valkyrie` 作为平行后缀）。
- **`binary/`**: 存放可执行程序。
...
#### 本地依赖安装
在项目根目录下运行 `legion install`，它会根据 `legion.von` 或 `legions.von` 的定义，将所有声明的依赖下载并同步到项目的 `vendor/` 目录下。
