...
Valkyrie 采用结构化、国际化优先的诊断思路。下文的 `ValkyrieError` 草图表达 **设计意图**；具体类型名与模块以各 crate 现行
`miette`/`Diagnostic` 实现为准（工作区已无独立的 `valkyrie-error` crate）。
...
- **宏依赖政策**: 语言主链与 emitter 诊断面以显式实现为主。工作区根 `Cargo.toml` 仍声明 `thiserror` 供部分包管理/工具
  crate 使用—— **不得**把「全仓库严禁 thiserror」写成与 lockfile 矛盾的绝对禁令；新诊断代码优先 `miette`，避免再扩散
  `anyhow` 风格上下文丢失。
...
- **宏与手写 Diagnostic**: 新诊断优先 `miette` + 显式 `Diagnostic`/`Display`。部分工具 crate 已使用 `thiserror`（见根
  `Cargo.toml`）——不必为「绝对禁止」而强行回写；主链新代码仍优先显式实现。
