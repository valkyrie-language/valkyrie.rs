...
> **设计笔记**：Valkyrie 严格区分了**状态叠加**与**属性扩展**。`|` 符号现在专门用于并集类型（Union Types），表示“或者是”的关系；而记录（Record）中的行扩展则采用 `, ...R` 语法，这与模式匹配中的对象展开操作保持语义一致。详见 [行类型与多态](./row-types.md#语法一致性与设计-syntax--consistency)。
