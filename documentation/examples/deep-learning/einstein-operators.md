...
# 多轴约简
let μ = reduce(𝐅, "n c h w -> c", "mean")  # 每个通道的均值
let 𝐌 = reduce(𝐅, "n c h w -> n c", "max")   # 空间最大值
...
let 𝐌_h = reduce(𝐓, "d h f -> h f", "max")    # 每小时最大值
```
...
let 𝐀 = repeat(𝐬, "h w c -> n h w c", n=8)  # 创建8个副本

# 位置编码重复
let 𝐄 = ArrayND::random([64, 512])  # sequence_length, d_model
let 𝐁 = repeat(𝐄, "s d -> n s d", n=32)  # 为整个batch重复

# 卷积核重复
let 𝐊 = ArrayND::random([3, 3])  # 2D卷积核
