...
在 OS 内核开发中，精确的内存控制至关重要。Valkyrie 允许在不使用 `class`（以及与之相关的垃圾回收和运行时元数据）的情况下，仅使用纯
`structure` 来实现完全的手动内存管理。
...
内核中部分结构需要在生命周期内保持固定地址，例如 DMA 缓冲区、指向自身或其他字段的裸指针、协程/中断帧内的自引用状态。Valkyrie
**不提供** `Pin<T>` 包装类型；在字段上使用 `[pin]` 标注，由编译器传播不可 Move 约束。
...
内核 profile 应使用纯 `structure` 与手动分配器，不引入 `class` 或 RC/GC 元数据。`[pin]` 仅约束 Move
语义与地址稳定，不解决循环引用；侵入式图结构仍由固定池与显式生命周期管理。
