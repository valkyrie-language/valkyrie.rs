...
widget TextInput {
    ...
    initiate(mut self, placeholder: utf8 = "") {
        self.value = ""
    ...
    # 链式配置
    maximum_length(mut self, length: usize) -> Self {
...
widget Container {
    ...
    # 添加子组件
    add_child(mut self, child: Box⟨Widget⟩) -> Self {
        self.children.push(child)
        self
    }
    # 批量添加子组件
    add_children(mut self, children: [Box⟨Widget⟩]) -> Self {
...
widget FlexBox {
    ...
    # 添加弹性子项
    add_flex_child(mut self, widget: Box⟨Widget⟩, grow: f32 = 0.0, shrink: f32 = 1.0, basis: FlexBasis = FlexBasis::Auto) -> Self {
...
widget Grid {
    ...
    # 添加网格子项
    add_grid_child(mut self, widget: Box⟨Widget⟩, column: GridPosition, row: GridPosition) -> Self {
...
widget Form⟨T⟩ {
    ...
    # 添加字段
    add_field(mut self, name: utf8, field: Box⟨Widget⟩) -> Self {
        self.fields.insert(name, field)
        self
    }
    # 添加验证器
    add_validator(mut self, field_name: utf8, validator: Validator) -> Self {
    ...
    validate(mut self) -> ValidationResult {
        ...
        loop (field_name, validators) in self.validators {
            let field_value = self.get_field_value(field_name)
            loop validator in validators {
...
widget VirtualList⟨T⟩ {
    ...
    render(self) -> Element {
        ...
        # 可见项目
        loop i in visible_start..visible_end {
    ...
    # 设置项目渲染器
    item_renderer(mut self, renderer: micro(T, usize) -> Box⟨Widget⟩) -> Self {
...
widget Modal {
    ...
    new(title: utf8, content: Box⟨Widget⟩) {
        self.visible = false
        self.title = title
...
widget Responsive {
    ...
    # 为不同断点设置组件
    for_breakpoint(mut self, breakpoint: utf8, widget: Box⟨Widget⟩) -> Self {
        self.children.insert(breakpoint, widget)
        self
    }
    # 设置断点
    breakpoint(mut self, name: utf8, width: f32) -> Self {
    ...
    update_breakpoint(mut self, window_width: f32) {
        ...
        loop (name, width) in self.breakpoints {
...
widget Animated {
    ...
    new(child: Box⟨Widget⟩, animation: Animation) {
        self.child = child
