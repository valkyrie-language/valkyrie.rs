...
> 除 FFI 文档与 `[com]` 注解外，凡写“动态派发”“胖指针”“消除虚表开销”等，在 Valkyrie 语境下均指 **witness table**，不得写成 VTable 或 vtable。

### 见证表 (Witness Table) 与传统 OOP vtable 的区别

虽然见证表和传统的虚函数表 (OOP vtable) 都用于实现多态，但它们在设计哲学和内存布局上有显著区别。本节**不**讨论 Windows `[com]` 使用的 COM vtable。
