...
随着抽象需求的增加，具名 `trait` 可以包含关联类型、继承其他 Trait 或添加约束。这些能力都依赖 trait 的协议身份，因此不属于匿名
row。
...
- **接口是存在量化 ($\exists T$)**: 构造者拥有选择 $T$ 的权力，而调用者只知道“存在一个类型 $T$
  满足某些约束”，但并不知道 $T$ 具体是谁。
...
- **全动态支持**: 这种设计使得 Valkyrie 的 Trait 对象 **没有所谓的“对象安全”限制**。即使是返回 `Self`
  的方法，也可以在动态分发中安全使用，因为运行时可以通过见证表动态处理返回值。
...
### 术语区分：witness table、COM vtable、传统 vtable

Valkyrie 文档与编译器内部必须区分三种易混淆概念：

| 术语                        | 层级                   | 说明                                                                                                                                 |
|:----------------------------|:-----------------------|:-------------------------------------------------------------------------------------------------------------------------------------|
| **witness table（见证表）** | 语言 / `trait`·`imply` | Valkyrie 的动态派发机制。对象布局不变；trait 对象用胖指针 `(data, witness)` 携带外部见证表。编译器优化中的“见证消除”指消除此类派发。 |
| **COM vtable**              | Windows FFI（`[com]`） | COM 接口 ABI：接口指针指向的虚函数表，按槽位调用 `IUnknown::Release` 等。**仅用于外部互操作**，与 `imply` 无关。                     |
| **传统 OOP vtable**         | 对比参照               | C++/Java 式侵入式类虚表。用于下文对比，**不是** Valkyrie 对 `class` / `trait` 的实现选择。                                           |

> 除 FFI 文档与 `[com]` 注解外，凡写“动态派发”“胖指针”“消除虚表开销”等，在 Valkyrie 语境下均指 **witness table**，不得写成
> VTable 或 vtable。

### 见证表 (Witness Table) 与传统 OOP vtable 的区别

虽然见证表和传统的虚函数表 (OOP vtable) 都用于实现多态，但它们在设计哲学和内存布局上有显著区别。本节 **不**讨论 Windows
`[com]` 使用的 COM vtable。
...
4. **按需分发**:
   只有在代码真正需要将对象作为 `Trait 对象` 处理时，才会生成胖指针。在普通调用中，完全没有见证表带来的间接开销。
