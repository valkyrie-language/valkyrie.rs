...
Valkyrie 的属性系统提供了一种优雅的方式来封装字段访问，允许开发者定义 getter 和 setter
方法，同时保持简洁的访问语法。属性系统遵循统一访问原则，使得字段访问和方法调用在语法上保持一致。

## 基本属性定义 (Basic Property Definitions)

### Getter 属性 (Getter Properties)
...
class DataProcessor {
    ...
    micro process_item(self, item: &utf8) -> ProcessedItem {
        # 复杂的处理逻辑
...
class Builder {
    ...
    email: utf8?,
    # 链式 setter
    set name(mut self, value: utf8) -> Self {
    ...
    set email(mut self, value: utf8) -> Self {
...
### 1. 使用属性进行数据封装 (Use Properties for Data Encapsulation)
...
### 2. 避免副作用过大的 Getter (Avoid Heavy Side-effects in Getters)

```valkyrie
# 避免：getter 中有复杂的副作用
class BadExample {
    get data(mut self) -> [utf8] {
        # 不好：每次访问都重新计算
...
### 3. 保持属性语义的一致性 (Maintain Property Semantic Consistency)
