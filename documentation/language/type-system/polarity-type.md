...
在代数子类型 (Algebraic Subtyping) 理论中，型变 (Variance) 不是孤立规则，而是 **类型极性 (Type Polarity)**
的直接体现。极性用于说明协变、逆变与极点类型之间的关系。
...
当一个泛型参数 `T` 同时出现在输入位置（`push(T)`）和输出位置（`get() -> T`）时，其极性相互抵消，最终表现为 **不变性 (
Invariance)**。
