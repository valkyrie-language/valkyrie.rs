...
联合类型用于表示多种可能值，并使用 `unite` 定义。`unite` 的默认表示是抽象类；`[tag(XXXKind)]` 是可选优化，用于要求 tagged
union；少数特例还会走利基优化，但通常不需要在前端单独处理。
...
也就是说，`Some(xxx)`、`Fine(xxx)` 作为 **构造 / 匹配语法**可以是合法的；关键在于不要把它误解成“variant 只能以 tuple-style
声明”。
