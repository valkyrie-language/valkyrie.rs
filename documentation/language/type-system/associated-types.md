...
关联类型是 Valkyrie 具名 `trait` 系统中的高级抽象机制。它允许你在 `trait` 定义中声明一个占位类型，该类型的具体选择将由
`trait` 的实现者决定。
...
关联类型将“类型占位符”与具名 `trait` 绑定在一起。相比于泛型参数，关联类型更强调一种 **函数式映射关系**：对于每一个实现该
`trait` 的具体类型，关联类型都有唯一确定的输出。
...
- **关联类型**：一个类型只能为给定的 `trait` 提供 **一个**实现。例如，`[i32]` 只能有一个 `Iterator` 实现，其 `Item` 必须是
  `i32`。
- **泛型参数**：一个类型可以为同一个 `trait` 提供 **多个**实现。例如，`Data` 类型可以同时实现 `Convert⟨i32⟩` 和
  `Convert⟨utf8⟩`。
...
如果你需要表达 `next()` 返回的元素类型在系统中继续传播，应优先写具名 `trait`，而不是试图把匿名 row 扩展成半套 trait
system。
