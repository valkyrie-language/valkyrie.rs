...
线性类型系统（Linear Type System）用于处理资源管理和并发安全。它要求一个变量**必须且只能被使用一次**。这比 Rust 的仿射类型（Affine Types，即变量最多被使用一次）更严格。
...
- **销毁钩子**：当 Nyar GC 回收不再可达的 `Continuation` 对象时，会调用其见证表中的 `drop_and_dealloc` 函数。
