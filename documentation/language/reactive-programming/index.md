...
> **状态：规划中。** 该能力尚未在编译器/运行时中实现。下文为设计意图。当前语言要求显式 `.await` / `.awake` / `.block`
> 后缀语法；不存在隐式自动 await 插入。
...
> **状态：部分落地。** 标准库中已存在 `CancellationToken` 类型与 `Promise.cancel(mut self) -> unit`，但仅作为基本取消标志位。
> `Promise.cancellable` 构造器、运行时有效的取消（真正中断运行中的异步任务）、通过 `.await` / `.block` 的取消传播，以及取消感知的
> resume 行为均尚未实现。`token.cancel()` 目前仅置位标志，并不会中断运行中的任务。
...
Promise 作为 Future 的唯一实现，提供了统一的 `.await` / `.awake` / `.block` 执行控制。（取消功能已规划但尚未完整实现——见上文的
*Promise 取消功能* 一节。）

### 与现有 await 语法的关系

> **状态：规划中（自动 await）。** 异步函数内的隐式自动 await 尚未在编译器/运行时中实现。当前语言要求对每个异步结果显式书写
> `.await`，即便在 `async { }` 块内也是如此。
