...
> **注意**：Valkyrie 不提供 `extern` / `unsafe` 关键字。外部符号通过 `@import` / `↯import` / `[wasi(...)]` 等属性声明；原始指针通过 `◇T` / `◆T` 与后缀解引用（`.◇` / `.◆`）表达。

> **注意**：本页主要介绍 FFI 的语法和调用方式。关于如何安全地管理外部对象的生命周期（如自动释放内存），请参考 [外部对象生命周期管理](../lifetime/foreign-objects.md)。
...
# Valkyrie 包装器 (Reference Class)
class Vector3D {
    ptr: ◆u8
}

imply Vector3D {
    initiate(mut self, x: f64, y: f64, z: f64) {
...
self.ptr = vector3d_new(x, y, z)
    ...
    micro magnitude(self) -> f64 {
         vector3d_magnitude(self.ptr) 
    }
    
    micro normalize(mut self) {
         vector3d_normalize(self.ptr) 
    }
    
    micro dot(self, other: Vector3D) -> f64 {
         vector3d_dot(self.ptr, other.ptr) 
    }
...
micro rust_sort_array(arr: ◆i32, length: usize)
...
micro use_dynamic_library() -> Result⟨(), Error⟩ {
    ...
    # 获取函数指针
    let add_func: Symbol⟨micro(i32, i32) -> i32⟩ = lib.get_function(b"add")?
    let multiply_func: Symbol⟨micro(f64, f64) -> f64⟩ = lib.get_function(b"multiply")?
    # 调用动态加载的函数
...
imply PluginManager {
    ...
    micro load_plugin(mut self, path: utf8) -> Result⟨(), Error⟩ {
            let lib = Library::new(path)?
            # 获取插件创建函数
            let create_plugin: Symbol⟨micro() -> ◆Plugin⟩ = 
...
unite FFIError {
    NullPointer
    InvalidUtf8 { error: Error }
    LibraryLoadError { message: utf8 }
    SymbolNotFound { name: utf8 }
    FunctionCallFailed { code: i32 }
...
imply FFIError: Display {
    micro fmt(self, f: Formatter) -> Result {
        match self {
            ...
            FFIError::InvalidUtf8 { error: e } => f.write("Invalid UTF-8: {}".format(e)),
            FFIError::LibraryLoadError { message: msg } => f.write("Library load error: {}".format(msg)),
...
通过终结器 (Finalizer) 确保资源在超出作用域时自动释放。
