...
> **注意**：Valkyrie 不提供 `extern` / `unsafe` 关键字。外部符号通过 `@import` / `↯import` / `[wasi(...)]` 等属性声明；原始指针通过
> `◇T` / `◆T` 与后缀解引用（`.◇` / `.◆`）表达。

> **注意**：本页主要介绍 FFI
> 的语法和调用方式。关于如何安全地管理外部对象的生命周期（如自动释放内存），请参考 [外部对象生命周期管理](../lifetime/foreign-objects.md)。
