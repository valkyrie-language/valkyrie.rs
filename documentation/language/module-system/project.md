...
Project 模式是 Valkyrie 语言的单项目管理模式，通过 `legion.json` 文件定义项目配置。当目录中存在 `legion.json` 文件且没有
`legions.json` 文件时，该目录被识别为独立项目模式。
...
├── legion.json           # 项目配置文件
├── library/
│   └── _.v             # 建议：命名空间根目录定义（Valkyrie 源码，后缀 .v）
├── binary/
│   ├── main.v          # 注册为指令 `main`
│   ├── tool1.v         # 注册为指令 `tool1`
│   └── tool2/
...
# 运行二进制程序
v main                     # 运行 binary/main.v
v tool1                    # 运行 binary/tool1.v
v tool2                    # 运行 binary/tool2/_.v
```
