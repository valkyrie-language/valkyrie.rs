...
`break`，否则继续执行下一个 arm。由于存在 fallthrough，`case` 是语句而非表达式，不返回值。
...
> **状态**: 已落地。语句语义与 fallthrough 在 MIR
> 中已显式化；后端在主路径下行为正确。更多模式形式见 [模式匹配](../function-oriented/pattern-match.md)。
