...
Valkyrie 提供了完整的遊戲開發框架，支持2D/3D遊戲、實時渲染、物理模擬、音频處理、輸入管理等功能，為遊戲開發者提供高性能的開發體验。
...
Valkyrie 原生支持圖形編程，可以直接编寫 shader 代碼替代 GLSL，並提供完整的 wgpu 集成。

- [圖形編程與 Shader 開發](graphics-shader.md) - 完整的圖形編程指南，包括 shader 编寫、wgpu 集成、高級渲染技術等
- [GPU 計算與並行編程](gpu-compute.md) - GPGPU 編程、並行算法、物理模擬、機器學習加速等
...
Valkyrie 遊戲開發框架提供了完整的工具鏈，從底層的渲染和物理系統到高級的遊戲邏輯組织，支持快速原型開發和高性能遊戲制作。