...
features:
  ...
  - icon: 🚀
    title: 多目标编译
    details: 编译到 WebAssembly、JavaScript 和原生代码，一套代码运行在浏览器、服务器和桌面环境，真正实现"一次编写，到处运行"。
  - icon: ⚡️
    title: 函数式编程
  ...
  - icon: 🌐
    title: 渐进式采用
    details: 可与现有 JavaScript/TypeScript 项目无缝集成，支持渐进式迁移，降低学习成本和迁移风险。
---
...
- **多目标编译**: 编译到 WebAssembly、JavaScript 和原生代码
...
micro fetch_user_data(id: i32) -> User {
    let response = raise HttpRequest { method: "GET", url: "/api/users/{id}" };
    parse_json(response);
};
...
micro main() {
    try {
        ...
        case HttpRequest { method: "GET", url }:
            resume(http_client.get(url));
        case HttpRequest { method: "POST", url, body }:
            resume(http_client.post(url, body));
    };
};

// 模式匹配和类型安全
match user_result {
    case Some { u } if u.age >= 18: print("成年用户: {u.name}");
    case Some { u }: print("未成年用户: {u.name}");
    case None: print("用户不存在");
};
```
